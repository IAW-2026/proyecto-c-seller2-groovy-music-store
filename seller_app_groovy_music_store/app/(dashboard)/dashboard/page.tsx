...
export default async function DashboardPage() {
  ...
  const [totalProductos, totalVentas, ventasPendientes, ventasEnviadas, ingresos] =
    await Promise.all([
  ...
  const totalIngresos = ingresos._sum.precio_unitario ?? 0;
  ...
  return (
    <div>
        ...
        {stats.map((stat) => (
          ...
          >
            <div className="flex items-center justify-between mb-3">
              <p className="font-dm text-xs text-medium uppercase tracking-wide">
                {stat.label}
              </p>
      ...
      <div className="bg-foreground rounded-xl p-6 text-white">
        <p className="font-dm text-xs text-white/50 uppercase tracking-wide mb-2">
          Ingresos totales
        </p>
